 ...
 */
 ...
 void periodicWakeup(TimerHandle_t unused)
 {
   ...
   // Alternatively, you could use xSemaphoreGive(wakeSemaphore); if not in ISR.
 }
 ...
 {
   // Print a message indicating the intended sleep duration based on the value.
   switch (sleepTime)
   {
   ...
   // Note: The semaphore wait is commented out here.
   ...
   for (;;)
   {
     ...
     while (xQueueReceive(commandQueue, &eventType, 0) == pdTRUE)
     {
       ...
       case EVENT_LED:
         ...
         // RGB handling logic (to be implemented).
         break;
       case EVENT_RB_LED:
     ...
     // Optional delay can be added if needed.
 ...
 void loop()
 {
   if (xSemaphoreTake(wakeSemaphore, portMAX_DELAY) == pdTRUE)
   {
     ...
     case MODE_LIVE_TRACKING:
       ...
       queEvent();
       vTaskDelay(TICKS(100));    // Check queue for events.
       // xSemaphoreGive(wakeSemaphore);
       xSemaphoreTake(wakeSemaphore, 10);
